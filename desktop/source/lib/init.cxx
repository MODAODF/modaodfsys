...
static int doc_saveAs(LibreOfficeKitDocument* pThis, const char* sUrl, const char* pFormat, const char* pFilterOptions)
{
    ...
    try
    {
        ...
        bool bWatermarkDiagonal = false; // 預設非對角浮水印
        ...
        for (const auto& rOption : aOptionSeq)
        {
            ...
            else if (rOption == "WatermarkDiagonal")
                bWatermarkDiagonal = true;
            else
        ...
        if(!watermarkText.isEmpty())
        {
            ...
            aFilterData[ 1 ].Name = "WatermarkAngle";
            aFilterData[ 1 ].Value <<= (bWatermarkDiagonal ? 450 : 0);
...
// 設定哪些 uno 指令需要主動回報狀態，各指令間用 ',' 逗號區隔，
// 指令間不能有空白，需用 url encode。
// 指令前的 '.uno:' 可不加。
static void doc_getUnoStates(const char* pCommands)
{
    comphelper::ProfileZone aZone("doc_getUnoStates");
    ...
    if (!pViewShell && !pViewFrame)
    {
        SAL_WARN("lok", "getUnoStates: No Frame-Controller created.");
        return;
    }
    ...
    if (!xContext.is())
    {
        SAL_WARN("lok", "getUnoStates: Component context is not available");
        return;
    }

    const char* pCmd = pCommands + 13; // 跳過前導指令 "getunostates "
    std::vector<OUString> aUnos;

    // ',' delimited options
    aUnos = comphelper::string::split(OUString(pCmd, strlen(pCmd), RTL_TEXTENCODING_UTF8), ',');
    ...
    for (auto &it : aUnos)
    {
        ...
        if (pSlot)
            ...
            std::cerr << "'" << aCommandURL.Complete << "' not supported!\n";
    }
}
...
static char* doc_getCommandValues(LibreOfficeKitDocument* pThis, const char* pCommand)
{
    ...
    // Add by Firefly <firefly@ossii.com.tw>
    // 開頭是 "getunostates "表示要設定自動回報狀態的 UNO 指令
    if (!strncmp(pCommand, "getunostates ", 13))
    {
        doc_getUnoStates(pCommand);
        return nullptr;
    }
