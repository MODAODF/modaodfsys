...
$(foreach product,$(PRODUCTLIST),\
	...
	$(product)-freedesktop-menus_$(PRODUCTVERSION)-$(LIBO_VERSION_PATCH)_all.deb) \
))

$(deb_WORKDIR)/%-desktop-integration.tar.gz: $(deb_WORKDIR)/%-freedesktop-menus_$(PRODUCTVERSION)-$(LIBO_VERSION_PATCH)_all.deb
...
$(deb_WORKDIR)/%/DEBIAN/control: $(deb_SRCDIR)/control $(call gb_CustomTarget_get_workdir,sysui/share)/%/create_tree.sh
	...
	cat $< | tr -d "\015" | \
		...
		>> $@
	echo "Version: $(PRODUCTVERSION)-$(LIBO_VERSION_PATCH)" >>$@
