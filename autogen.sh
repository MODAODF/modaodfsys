...
if (defined $ENV{NOCONFIGURE}) {
    ...
    if (-e "Makefile"){
        my $str = `awk '/patch translations/{print}' Makefile`;
        my @flag = split(' ', $str);
        if (length($flag[2]) ne 12){
            system("sed -i '/make sure the git hooks are in place even if no submodules are needed/a # patch translations\\n\\t./translations_patch.sh 9' Makefile");
        }
    }
}
